...
export default function TeacherLoading() {
  return (
    <div
      className="p-6 md:p-10 space-y-8 animate-in fade-in duration-300"
      dir="rtl"
    >
      {/* ── Breadcrumb skeleton ─────────────────────────────── */}
      ...
      <div className="rounded-2xl border border-gray-100 dark:border-border bg-white dark:bg-card overflow-hidden">
        {/* Table header */}
        <div className="flex items-center gap-4 px-6 py-4 border-b border-gray-50 dark:border-border">
          <div className="h-4 w-32 rounded bg-gray-200 dark:bg-muted animate-pulse" />
          <div className="h-4 w-24 rounded bg-gray-100 dark:bg-muted/60 animate-pulse" />
          <div className="h-4 w-20 rounded bg-gray-100 dark:bg-muted/60 animate-pulse ml-auto" />
        </div>
        {/* Table rows */}
        {Array.from({ length: 5 }).map((_, i) => (
          <div
            key={i}
            className="flex items-center gap-4 px-6 py-4 border-b border-gray-50 dark:border-border last:border-b-0"
          >
            <div className="h-9 w-9 rounded-full bg-gray-200 dark:bg-muted animate-pulse shrink-0" />
            <div className="space-y-2 flex-1">
              <div className="h-4 w-40 rounded bg-gray-200 dark:bg-muted animate-pulse" />
              <div className="h-3 w-24 rounded bg-gray-100 dark:bg-muted/60 animate-pulse" />
            </div>
            <div className="h-6 w-16 rounded-full bg-gray-100 dark:bg-muted/60 animate-pulse" />
            <div className="h-8 w-20 rounded-lg bg-gray-100 dark:bg-muted/60 animate-pulse" />
          </div>
        ))}
      </div>
    </div>
